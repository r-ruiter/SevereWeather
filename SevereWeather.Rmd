...
The resulting table contains now `r length(unique(dfEvtype$EVTYPE))` event types, the ones that doesn't occur in the event type table from the NWS are not disturbing the results. There is no considerable damage or harm for those types.
...
```{r plot1, fig.cap="*Fig 1. Damage per Event Type*", fig.height=5, fig.width=8, echo=FALSE}
...
```{r plot2, fig.cap="*Fig 2. Fatalities and Injuries per Event Type*", fig.height=5, fig.width=8, echo=FALSE}
